...
export default function PostDetails() {
  ...
  useEffect(() => {
    fetch(`https://blogappapi-i362.onrender.com/blogs/${postId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data._id) {
          setPost(data);
  ...
  function deletePost() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://blogappapi-i362.onrender.com/blogs/${postId}/delete`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  function addComment() {
    ...
    fetch(`https://blogappapi-i362.onrender.com/blogs/${postId}/comment`, {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  function deleteComment(commentId) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://blogappapi-i362.onrender.com/blogs/${postId}/deletecomment/${commentId}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
